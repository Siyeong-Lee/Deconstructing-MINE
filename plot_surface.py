...
import MINE.dataloader
import MINE.model_loader
import MINE.evaluation
from MINE.losses import MI_BasicLoss, MI_MALoss, MI_SHLoss
...
def crunch(surf_file, net, w, s, d, dataloader, loss_key, acc_key, comm, rank, args):
    ...
    if args.loss_name == 'basic':
        criterion = MI_BasicLoss(args.batch_size)
    elif args.loss_name == 'MINE_stable':
        criterion = MI_MALoss(args.batch_size)
    elif args.loss_name == 'MINE_SH':
        criterion = MI_SHLoss(args.batch_size)
    else:
...
if __name__ == '__main__':
    ...
    # model parameters
    parser.add_arguemnt('--num_classes', default=4, type=int, help='number of classes')
    parser.add_argument('--model', default='concat', help='model name')
    ...
    #--------------------------------------------------------------------------
    net = model_loader.load(args.model, args.num_classes, args.model_file)
    ...
    # Setup dataloader
    #--------------------------------------------------------------------------
    loader = dataloader.load_dataset(args.batch_size, args.num_classe,
        1000, True, args.threads)

    #--------------------------------------------------------------------------
    # Start the computation
    #--------------------------------------------------------------------------
    crunch(surf_file, net, w, s, d, loader, 'loss', 'mi', comm, rank, args)
    ...
    if args.plot and rank == 0:
        if args.y and args.proj_file:
            plot_2D.plot_contour_trajectory(surf_file, dir_file, args.proj_file, 'loss', args.show)
        elif args.y:
